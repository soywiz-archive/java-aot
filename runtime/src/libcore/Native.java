...
public class Native {
     @MethodBody("wchar_t libcore_Native::lower(wchar_t v) { return ::towlower(v); }") static public native char lower(char v);
	//static public char lower(char v) { AOT.cpp("return ::towlower(v);"); return 0; }

    @MethodBody("wchar_t libcore_Native::upper(wchar_t v) { return ::towupper(v); }")
	static public native char upper(char v);

    @MethodBody("float64 libcore_Native::atan(float64 v) { return ::atan(v); }")
    static public native double atan(double v);

    @MethodBody("float64 libcore_Native::cos(float64 v) { return ::cos(v); }")
    static public native double cos(double v);

    @MethodBody("float64 libcore_Native::sin(float64 v) { return ::sin(v); }")
    static public native double sin(double v);

    @MethodBody("float32 libcore_Native::intBitsToFloat(int32 v) { return *(float32*)&v; }")
    static public native float intBitsToFloat(int v);

    @MethodBody("bool libcore_Native::isNaN(float32 v) { return isnan(v); }")
    static public native boolean isNaN(float v);

    @MethodBody("void libcore_Native::putchar(wchar_t v) { ::putwchar(v); }")
    static public native void putchar(char v);

    @MethodBody("void libcore_Native::flush() { ::fflush(stdout); }")
    static public native void flush();

    @MethodBody("void libcore_Native::debugint(int v) { ::printf(\"Int(%d)\", v); }")
    static public native void debugint(int v);

    @MethodBody("void libcore_Native::exit(int32 status) { ::exit(status); }")
    ...
    @MethodBody("void libcore_Native::gc() { }")
	static public native void gc();

    @MethodBody("int64 libcore_Native::currentTimeMillis() { struct timeval tp; ::gettimeofday(&tp, NULL); int64 result = (int64)tp.tv_sec * 1000L + (int64)tp.tv_usec / 1000L; return result; }")
