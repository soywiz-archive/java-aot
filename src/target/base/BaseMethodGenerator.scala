...
abstract class BaseMethodGenerator(method: SootMethod, mangler: BaseMangler) {
  ...
  def doMethod(): MethodResult = {
    ...
    for (local2 <- locals) {
      ...
      bodyString += mangler.typeToCppRef(local.getType) + " " + name + ";\n"
    }
    bodyString += stms
  ...
  def doExpr(expr: Expr): String = {
    expr match {
      ...
      case e: CastExpr =>
        ...
        "((" + mangler.typeToCppRef(e.getCastType) + ")" + doValue(e.getOp) + ")"
      case e: InstanceOfExpr =>
        referenceType(e.getType)
        e.getOp + " instanceof " + mangler.typeToCppRef(e.getType)
      case e: NewExpr =>
        referenceType(e.getType)
          ...
          case i: InstanceInvokeExpr =>
            i match {
              ...
              case i: SpecialInvokeExpr =>
                doValue(i.getBase) + "->" + mangler.mangle(i.getMethod.getDeclaringClass) + "::" + mangleBaseName(e.getMethod) + "(" + argsCall + ")"
            }
        }
